...
class SalesforceManager:
    ...
    def extract_case_info(self):
        ...
        email = ""
        try:
            case_number = self.driver.find_elements(By.XPATH, "(//records-record-layout-item[@field-label="
                                                              "'Case Number']//div//div//div)[2]")[0].text
            email = self.driver.find_elements(By.XPATH,"(//span[.='Account Name']/../..//div)[2]//a//span")[0].text
    ...
    def search_case(self, case_number):
        ...
        id = self.execute_on_web_element("(//div[@class='pbBody']//tr)[2]//th//a",
    ...
    def send_confirmation_email(self, case_number):
        self.search_case(case_number)
        package_info = self.extract_case_info()
        self.driver.get(f"https://autelroboticsusa.my.salesforce.com/_ui/core/email/author/EmailAuthor")
        ...
        # From
        Select(self.execute_on_web_element("//select[@id='p26']", lambda f: f)).select_by_index(1)

        # Subject
        self.execute_on_web_element("//input[@id='p6']", lambda f: f.send_keys(subject), 0)

        # Body
        self.execute_on_web_element("//textarea[@id='p7']", lambda f: f.send_keys(body), 0)

        # Email
        self.execute_on_web_element("//textarea[@id='p24']", lambda f: f.send_keys(package_info.email), 0)

        # Related To
        Select(self.execute_on_web_element("//select[@id='p3_mlktp']", lambda f: f)).select_by_index(15)

        # Case Number
        self.execute_on_web_element("//input[@id='p3']", lambda f: f.send_keys(case_number), 0)

        # Click the send button
        self.execute_on_web_element("//input[@title='Send']", lambda f: f.click(), 0)


# manager = SalesforceManager()
# manager.login()
# set_case_number = "00057550"
# manager.send_confirmation_email(set_case_number)
