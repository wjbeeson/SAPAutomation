...
class ReceiveForm(FormBase):
    def __init__(self, driver=None, package_info: PackageInfo = PackageInfo()):
        ...
        # Form Numbers
        Label(self.base, text=f"ZT01 Number", font=("bold", 10)).grid(row=current_row)
        self.zt01_number = Entry(self.base)
        self.zt01_number.grid(row=current_row, column=1)
        current_row += 1

        Label(self.base, text=f"VL01N Number", font=("bold", 10)).grid(row=current_row)
        self.vl01n_number = Entry(self.base)
    ...
    def send_email(self):
        self.format_form()
        send_confirmation_email(self.driver, self.case_number.get())
